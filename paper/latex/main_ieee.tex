...
\begin{table*}[t]
\centering
\caption{Multi-Domain Benchmark Evaluation Across Baseline Strategies (2,500 Test Queries, 500 per Domain, Across 3 Random Seeds on NVIDIA T4 Hardware; Reported as Mean $\pm$ Standard Deviation).}
...
Fig.~\ref{fig:router_heatmap} plots the routing allocation matrix. The router demonstrates sharp domain discernment: 68.0\% of GSM8K prompts route to $E_{\text{math}}$ ($E_1$), 60.0\% of MBPP prompts route to $E_{\text{code}}$ ($E_2$), and 82.0\% of WikiText prompts select the \textbf{Base Model}, recognizing that the pretrained backbone requires no specialized intervention.
