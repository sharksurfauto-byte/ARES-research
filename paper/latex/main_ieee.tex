...
\thanks{A. Jawadwala is with the School of Computing Sciences, Hindustan Institute of Technology and Science, Chennai, Tamil Nadu 603103, India (e-mail: 24cu0330065@student.hindustanuniv.ac.in). Manuscript received September 2026.}}
...
Existing methodologies for adapting language models to complex specialized domains impose steep operational tradeoffs:
\begin{enumerate}
...
We compared three candidate probe architectures on identical harvested representation tensors:
\begin{table}[h]
\centering
...
\includegraphics[width=\textwidth]{figures/fig2_pareto_frontier.png}
\caption{Accuracy vs. Compute Pareto Frontier.}
\label{fig:pareto}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\textwidth}
\includegraphics[width=\textwidth]{figures/fig4_risk_coverage.png}
\caption{Selective Prediction Risk-Coverage Curves (AURC).}
...
The frozen base model (B0) exhibits pronounced deficiencies on formal quantitative reasoning (32.0\% on GSM8K, 36.0\% on MBPP). While always-on MoE (B3) elevates aggregate accuracy to 62.40\%, it incurs a 100\% expert invocation overhead. **ARES (B4) attains 61.20\% overall accuracy**, capturing **98.1\% of peak MoE capability** while invoking specialized experts on only **41.6\% of queries**, realizing a **58.4\% compute savings**.

\begin{figure*}[t]
\centering
\begin{subfigure}[b]{0.48\textwidth}
\includegraphics[width=\textwidth]{figures/fig3_calibration_ece.png}
\caption{Dual Reliability Diagrams (Raw vs. Calibrated).}
\label{fig:calibration}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\textwidth}
\includegraphics[width=\textwidth]{figures/fig6_router_distribution.png}
\caption{Router Dispatch Matrix Heatmap.}
...
As shown in Fig.~\ref{fig:calibration}, the calibration gap of the raw probe ($ECE = 0.1911$) collapses to **$ECE = 0.0480$** following isotonic regression. This confirms **Hypothesis 3**, establishing that ARES reliability scores correspond to dependable error probabilities.

\subsection{Selective Prediction (AURC)}
Fig.~\ref{fig:risk_coverage} depicts selective error risk as a function of query coverage. ARES achieves an **Area Under the Risk-Coverage curve (AURC) of 0.284**, substantially outperforming the base model ($\text{AURC} = 0.458$) and token entropy ($\text{AURC} = 0.402$). At 80\% coverage, ARES slashes the selective error rate by **12.7\% absolute**.
...
\begin{itemize}
    ...
    \item 82.0\% of WikiText prompts select the **Base Model**, recognizing that the pretrained backbone requires no specialized intervention.
\end{itemize}
...
\end{itemize}
Reliability probing and routing add **less than 6 ms of latency overhead**, representing $< 0.25\%$ of total generation latency.

\section{Engineering Challenges \& Retrospective}

\subsection{Adapter Dimension Incompatibility Across Scales}
When scaling from Qwen2.5-0.5B ($d=896$) to Qwen2.5-7B ($d=3584$), standard PEFT loaders silently instantiate new uninitialized random adapter weights when encountering target dimension discrepancies. We integrated strict validation in `ARESPipeline._load_components()` that verifies:
\begin{equation}
...
This paper introduced \textbf{ARES (Adaptive Reliability with Expert Specialization)}, an adaptive computation framework that couples internal representation probing with domain-specialized LoRA experts over frozen language models. By combining macro-level feasibility (GRM) with sequential token-level risk (LRM), ARES attains **61.20\% accuracy** across five challenging benchmarks, matching **98.1\%** of an unconstrained always-on MoE system while delivering a **58.4\% compute savings**. Furthermore, post-hoc isotonic calibration reduces ECE to **0.0480**, establishing dependable uncertainty boundaries.
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{figures/fig1_architecture.png}}]{Aliasghar Jawadwala}
