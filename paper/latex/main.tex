...
Evaluations span five distinct cognitive domains totaling \textbf{2,500 test queries (500 per domain)}:
\begin{enumerate}
    \item \textbf{Mathematics (GSM8K)}: 500 multi-step quantitative problem solving instances \cite{cobbe2021gsm8k}. Evaluated via strict numerical answer extraction.
    \item \textbf{Code Synthesis (MBPP)}: 500 Mostly Basic Python Problems \cite{austin2021mbpp}. Evaluated via programmatic assertion pass-rates.
    \item \textbf{Scientific Reasoning (AI2-ARC)}: 500 ARC Challenge grade-school science QA problems \cite{clark2018arc}.
    \item \textbf{Commonsense Reasoning (CSQA)}: 500 multi-choice commonsense associative reasoning items \cite{talmor2019commonsenseqa}.
    \item \textbf{General Fluency (WikiText-103)}: 500 continuous linguistic completion segments \cite{merity2017wikitext}.
\end{enumerate}
...
To foster open science and guarantee end-to-end reproducibility, all artifacts associated with this work---including the complete \texttt{ARESPipeline} implementation, probe and router training harnesses, evaluation scripts, and trained model checkpoints---are publicly available under the open-source MIT License at:
\begin{center}
\url{https://github.com/sharksurfauto-byte/ARES-research}
\end{center}
All benchmark evaluations can be replicated across random seeds ($\{42, 1337, 2026\}$) using the automated test suite (\texttt{ares\_full\_pipeline\_test.py}).
